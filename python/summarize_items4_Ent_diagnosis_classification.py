...
basic_list = ["pID","Age","Sex","Blood_ABO","Blood_Rh","department","height","weight","Ent_datetime",
			"Exit_datetime","days_of_stay","Ent_diagnosis_classification","Ent_diagnosis","Current_diagnosis","body_restraint","ambulance",
	...
	for d2 in dir2:
		...
		try:
			...
			for i in range(len(df)):
				...
				elif line[2] == "入室時診断分類コード":
					basic_info[case_id]["Ent_diagnosis_classification"] = val
				elif line[2] == "入室時診断":
...
with open("patient/ICU_patient_info_summarized4_Ent_diagnosis_classificartion.txt","w") as fo:
	# column name
...
with open("patient/ICU_patient_info_summarized_tested4_diagnosis_classificartion.txt","w") as fo:
	# column
	fo.write("pID")
...
with open("patient_error/ICU_patient_info_error2_diagnosis_classificartion.txt","w") as fo:
